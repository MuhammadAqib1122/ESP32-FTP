...
ESP32_FTPClient_updated::ESP32_FTPClient_updated(char* _serverAdress, uint16_t _port, char* _userName, char* _passWord, uint16_t _timeout, uint8_t _verbose) {
  userName = _userName;
  passWord = _passWord;
...
ESP32_FTPClient_updated::ESP32_FTPClient_updated(char* _serverAdress, char* _userName, char* _passWord, uint16_t _timeout, uint8_t _verbose) {
  userName = _userName;
  passWord = _passWord;
  serverAdress = _serverAdress;
  port = 21;
...
WiFiClient* ESP32_FTPClient_updated::GetDataClient() {
  return &dclient;
}

bool ESP32_FTPClient_updated::isConnected() {
  if (!_isConnected)
  {
    FTPerr("FTP error: ");
    FTPerr(outBuf);
...
void ESP32_FTPClient_updated::GetLastModifiedTime(const char  * fileName, char* result) {
  FTPdbgn("Send MDTM");
...
void ESP32_FTPClient_updated::WriteClientBuffered(WiFiClient* cli, unsigned char * data, int dataLength) {
  if (!isConnected()) return;
...
void ESP32_FTPClient_updated::GetFTPAnswer (char* result, int offsetStart) {
  char thisByte;
  outCount = 0;
...
void ESP32_FTPClient_updated::WriteData (unsigned char * data, int dataLength) {
  FTPdbgn(F("Writing"));
  if (!isConnected()) return;
  WriteClientBuffered(&dclient, &data[0], dataLength);
}

void ESP32_FTPClient_updated::CloseFile () {
...
void ESP32_FTPClient_updated::Write(const char * str) {
...
void ESP32_FTPClient_updated::CloseConnection() {
  client.println(F("QUIT"));
  client.stop();
  FTPdbgn(F("Connection closed"));
}

void ESP32_FTPClient_updated::OpenConnection() {
...
void ESP32_FTPClient_updated::RenameFile(char* from, char* to) {
  FTPdbgn("Send RNFR");
  if (!isConnected()) return;
  client.print(F("RNFR "));
...
void ESP32_FTPClient_updated::NewFile (const char* fileName) {
  FTPdbgn("Send STOR");
  if (!isConnected()) return;
  client.print(F("STOR "));
  client.println(F(fileName));
...
void ESP32_FTPClient_updated::InitFile(const char* type) {
  FTPdbgn("Send TYPE");
  if (!isConnected()) return;
  FTPdbgn(type);
...
void ESP32_FTPClient_updated::AppendFile (char* fileName) {
  FTPdbgn("Send APPE");
  if (!isConnected()) return;
  client.print(F("APPE "));
  client.println(F(fileName));
...
void ESP32_FTPClient_updated::ChangeWorkDir(const char * dir) {
  FTPdbgn("Send CWD");
  if (!isConnected()) return;
  client.print(F("CWD "));
...
void ESP32_FTPClient_updated::DeleteFile(const char * file) {
  FTPdbgn("Send DELE");
  if (!isConnected()) return;
  client.print(F("DELE "));
...
void ESP32_FTPClient_updated::MakeDir(const char * dir) {
  FTPdbgn("Send MKD");
  if (!isConnected()) return;
  client.print(F("MKD "));
  client.println(F(dir));
...
void ESP32_FTPClient_updated::ContentList(const char * dir, String * list) {
...
void ESP32_FTPClient_updated::DownloadString(const char * filename, String &str) {
  FTPdbgn("Send RETR");
  if (!isConnected()) return;
  client.print(F("RETR "));
  client.println(F(filename));
...
void ESP32_FTPClient_updated::DownloadFile(const char * filename, unsigned char * buf, size_t length, bool printUART ) {
  FTPdbgn("Send RETR");
  if (!isConnected()) return;
  client.print(F("RETR "));
  client.println(F(filename));
